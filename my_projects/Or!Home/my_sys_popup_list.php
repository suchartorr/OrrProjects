...
class my extends my_page {
    ...
    function popup() {
         ...
         */
        $sql = "SELECT *  FROM `my_sys`"; //<-กำหนดคำสั่ง SQL

        $my_form = new OrDbPopupList('my_form', $this->get_my_db());
        $my_form->OP_[edit_page_url]->set('my_sys.php'); //กำหนด URL ของหน้าแก้ไขข้อมูล
        $my_form->OP_[edit_field_link]->set('sys_id'); //กำหนด ชื่อ Field ที่ต้องการให้เป็น Link หนาแก้ไขข้อมูล
        $my_form->OP_[edit_key_field]->set('sys_id'); //กำหนด ชื่อ Field ที่เป็นคีย์แก้ไข
    ...
    function content($content_key_value) {
        ...
        $sql = "SELECT *  FROM `my_sys` WHERE `sys_id` = '" . $content_key_value . "'";
        $my_db->get_query($sql);
        if ($my_db->get_record()) {
            $my_value = $my_db->record['title'];
        } else {
